    ...
    for(let tx of json.result.slice(1)){
        ...
        if(tx.input == ""||tx.input == "0x"){
            ...
            if(option.all || option.printTx){
                ...
                +
                (input!=undefined? (input._heroType != undefined? input._heroType:""):"")
                , 
...
async function main(){
    ...
    let address = "0x946048A75Af11C300A274344887eC39452218B3d";
    let name = "HeroesPresale";
    await getSource(address, name);
    loadABI(name);
    await getTransactions(5096, address);
    await filter();
}
