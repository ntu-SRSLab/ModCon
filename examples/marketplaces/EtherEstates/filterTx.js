...
function loadABI(name){
    ...
    for (let abi of ABIs){
        if((false == abi.constant|| ("view"!=abi.stateMutability && "pure"!=abi.stateMutability) )&& abi.type == "function"){
    ...
    for(let tx of json.result.slice(1)){
        ...
        if(tx.input == ""||tx.input == "0x"){
            ...
            if(option.all || option.printTx){
                ...
                if (HashMethodMap[tx.input.substring(0,10)].name == "createMultiple"){
                    let itemIds = input._itemIds;
...
async function main(){
    ...
    let address = "0xe26b758708f39276f879c0a187d9a789fc26c38e";
    let name = "EtherEstates";
    await getSource(address, name);
    loadABI(name);
    await getTransactions(2395, address);
    await filter();
}
